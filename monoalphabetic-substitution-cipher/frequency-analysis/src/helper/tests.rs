...
fn test_get_next_most_frequent(
    letter_frequency: HashMap<char, u32>,
    expected_most_frequent_letter: char,
) {
    //when
...
fn test_replace_all_occurances(
    ...
    //when
    let returned_letter_frequency =
        replace_all_occurances(orginal, orginal.chars().collect(), replacing, replace_with);
